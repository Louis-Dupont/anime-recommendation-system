...
class RatingModel:
    ...
    def fit(self, data, standardize_input=False):
        if standardize_input:
            data = self.standardizer.transform_array(data)
        embedding_test = self.svd.fit(data)
        return self

    def predict(self, data, standardize_input=False, destandardize_output=True):
        if standardize_input:
            data = self.standardizer.transform_array(data)
    ...
    def mean_predict(self, data, destandardize_output=True):
        if not destandardize_output:
